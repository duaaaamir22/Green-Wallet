# ─────────────────────────────────────────────────────────
# app.py
# Contributor : Dua Amir
# Role        : Main Streamlit application — login screen,
#               data-fetch screen, and all 6 dashboard tabs
# Last commit : "feat: wire up all screens + simulator + chat UI"
# ─────────────────────────────────────────────────────────
    ...
    if st.button("Connect Portfolio"):
        if pin != "123456":
            ...
            if not user:
                ...
                for t, s in stocks:
                    q("INSERT INTO holdings(user_id,ticker,shares) VALUES(?,?,?)", (uid, t, s))
                user = q("SELECT * FROM users WHERE username=?", (name,), one=True)
            st.session_state.update(user=user, uid=user[0], screen="fetch")
            st.rerun()
...
# ═══════════════════════════════════════════════════════
    ...
    for i, h in enumerate(holdings):
        ...
        price = get_price(h[2])
        e, s, g, comp, src, sector, expl = get_esg(h[2])
    ...
    st.session_state.update(pdata=pdata, score=score, screen="app")
    time.sleep(0.5)
    st.rerun()
...
# ═══════════════════════════════════════════════════════
    ...
    with tabs[0]:
        ...
        if st.button("💾 Save Score to History"):
            q("INSERT INTO analytics(user_id,green_score) VALUES(?,?)", (uid, int(sc)))
            st.success("Saved!")
    ...
    with tabs[2]:
        ...
        s = next(x for x in pdata if x["ticker"] == sel)
        ...
        bar("🌍 Environmental", s["env"], G)
        bar("👥 Social",        s["soc"], "#00cec9")
        bar("🏛️ Governance",   s["gov"], "#6366f1")
    ...
    with tabs[3]:
        ...
        if data:
            ...
            st.info("No history yet. Save scores from Score tab.")
    ...
    with tabs[4]:
        ...
        diff = sim_sc - sc
        sl, scl, scol = tier(sim_sc)
        dc = G if diff > 0 else D if diff < 0 else MUTED
            ...
            for i, question in enumerate(["How can I improve my score?","Which stock is my biggest ESG risk?","Compare my portfolio to market benchmarks","What does my environmental impact look like?"]):
                if st.button(question, key=f"q{i}"):
                    with st.spinner("Thinking..."):
                        reply = ask_advisor(question, pdata, sc)
                    st.session_state.chat += [{"r":"u","t":question},{"r":"b","t":reply}]
                    st.rerun()
        for m in st.session_state.chat:
            st.markdown(f"<div class='chat-{'u' if m['r']=='u' else 'b'}'>{m['t']}</div>", unsafe_allow_html=True)
        inp = st.chat_input("Ask about your portfolio's ESG impact...")
        if inp:
            with st.spinner("Thinking..."):
                reply = ask_advisor(inp, pdata, sc)
            st.session_state.chat += [{"r":"u","t":inp},{"r":"b","t":reply}]
            st.rerun()
