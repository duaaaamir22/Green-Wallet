...
@st.cache_resource
def init_db():
    """Creates SQLite database with 4 tables: users, holdings, esg_cache, analytics."""
...
def q(sql, args=(), fetch=False, one=False):
    """Executes a SQL query and returns results based on fetch/one flags."""
    c = conn.cursor()
    c.execute(sql, args)
    conn.commit()
...
def get_esg(ticker):
    ...
    # 1. Check hardcoded real-world scores
